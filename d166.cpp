...
// 最後的 -1 表示輪入結束, 下一行輸出結果:  5 9 1 8 2 6 4 7 3
   ...
   /*
     sourcearray[0] 的值為: 1  ,   mappingtable[0] 的值 2 , 
     題目: 其bm代表意思為：數字m的位置前面有幾個比大個個數。
     數字: "1" 要放在某個位置, 前面要有 "2" 個, 比  "1"大的 位置, 也就是前面要有 2 個空格
     targetarray[2] : 該位置前面,有2個空格, 之後就有機會塞進比  "1"大的數值
     數值: sourcearray[0]:"1" , 放進 targetarray[2] 的空位, 得到結果 {0, 0, 1, 0, 0, 0, 0, 0, 0};
     ==> 換成程式如下: 
     =======================================================================
     i 目前的值為 : 0
     source_value = sourcearray[i]   ,  source_value 值為: 1
     empty_count = mappingtable[i]   ,  empty_count 值為: 2
     查看目前 targetarray[] 的空格, 找到 location_index 值:2 時, 前面是空格數量為: empty_count:2
     targetarray[location_index] =  source_value <-- 將 source_value 放進 targetarray[location_index] 該空格中
     output result : targetarray[] : {0, 0, 1, 0, 0, 0, 0, 0, 0}
     ----------------------------------------------------------------
     i 目前的值為 : 1
     source_value = sourcearray[i]   ,  source_value 值為: 2
     empty_count = mappingtable[i]   ,  empty_count 值為: 3
     查看目前 targetarray[] 的空格, 找到 location_index 值:4 時, 前面是空格數量為: empty_count:3
     (因為 targetarray[2] 不是空白值, 前一次塞值進去了  )
     targetarray[location_index] =  source_value <-- 將 source_value 放進 targetarray[location_index] 該空格中
     output result : targetarray[] : {0,　0, 1,　0, 2,　0,　0,　0,　0}
     ----------------------------------------------------------------
     i 目前的值為 : 2
     source_value = sourcearray[i]   ,  source_value 值為: 3
     empty_count = mappingtable[i]   ,  empty_count 值為: 6
     查看目前 targetarray[] 的空格, 找到 location_index 值:8 時, 前面是空格數量為: empty_count:6
     (因為 targetarray[2] & [4] 不是空白值, 前二次塞值進去了  )
     targetarray[location_index] =  source_value <-- 將 source_value 放進 targetarray[location_index] 該空格中
     output result : targetarray[] : {0,　0, 1,　0, 2,　0,　0,　0,　3}
     ----------------------------------------------------------------
     i 目前的值為 : 3
     source_value = sourcearray[i]   ,  source_value 值為: 4
     empty_count = mappingtable[i]   ,  empty_count 值為: 4
     查看目前 targetarray[] 的空格, 找到 location_index 值:6 時, 前面是空格數量為: empty_count:4
     (因為 targetarray[2] & [4] 不是空白值, 前二次塞值進去了  )
     targetarray[location_index] =  source_value <-- 將 source_value 放進 targetarray[location_index] 該空格中
     output result : targetarray[] : {0,　0, 1,　0, 2,　0,　4,　0,　3}
     ----------------------------------------------------------------
     i 目前的值為 : 4
     source_value = sourcearray[i]   ,  source_value 值為: 5
     empty_count = mappingtable[i]   ,  empty_count 值為: 0
     查看目前 targetarray[] 的空格, 找到 location_index 值:0 時, 前面是空格數量為: empty_count:0
     targetarray[location_index] =  source_value <-- 將 source_value 放進 targetarray[location_index] 該空格中
     output result : targetarray[] : {5, 0, 1,　0, 2,　0,　4,　0,　3}
     ----------------------------------------------------------------
     反覆往下作, 直到最後一個值
    */
